...
The energy resolution of the detector used in this work is 127 $\pm$6 eV at ICR between 1 k and 42 k cps, with the highest PT avaliable.
...
% TODO discuss: GaSb have more spectra at 15 and 30, which might be the reason for the better results.
...
% TODO discuss: even thought the detector is with a polymer (?) window for allowing low energy X-rays, the absorption is still high in the specimen (the window let the X-rays pass, but few are coming.)
...
% TODO discuss: why are these t-values good? compare to the range r, and comment that the increasing devaition with high t can be due to ...?
...
%TODO discuss: in general, the TEM results are poor, especially true for the AZtec results. The CL with absorption corrections show that absorption corrections can improve the results, but using TEM routines on SEM data is not recommended. Setting the best t is also not trivial.
% TODO discuss: To conclude, the TEM EDS quantification of SEM EDS data show that the routines are built on similar principles, but that the routines have differences which are important. Too unrelibale!
...
% TODO? illustrate the different r values? Also, r is different for kV values.
...
% TODO discuss: ZAF A implies that GaAs needs abs. corr, but that GaSb have issue with something else. Z is close for Ga and As, while far apart for Ga Sb. Thus, Z effects might not be negligble as stated in Goldstein.
% TODO discuss: the ZAF A corrections are not good enough, at lest with the average point method tested in this work.
% TODO discuss: mu_rho is taken from HS, and slight deviations can give large impacts, as mu_rho is in the exponent. ref to eq. in theory. 
...
% \input{tables/results/table_ZAF_A_range_r.tex} % Is now in appendix
...
% These correction factors were calculated with the XPP notebook, which is in \hyperref[appendix:xpp]{Appendix B}.
...
% TODO disucss: divide by F is a Z correction, and divide by f(chi) is both Z and A correction.
...
% TODO discuss: again, the GaSb is made much worse. Doing abs. corr if not needed seems to make the results bad, at lest with the avaliable input parameters used in this work.
% TODO discuss: XPP better than ZAF.
...
% TODO discuss: the right XPP corrections make the results better, but it may be that different kV and specimen (Z) should have different corrections applied. i.e. when Z is far apart, do Z corrections, but not if the lines chosen are x and y. cherry picking results?
% TODO discuss. but it is far from a finished results. Probably both a problem with the interpretation and the implementation of the XPP bulk corrections in the notebook. This can be investigated further, and the advice is to start with ...
