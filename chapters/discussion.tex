...
% TODO disc: sjekke "possibility", "may",  "might", "possibly". også "not"
...
% artifacts
The artifacts discovered are listed in \cref{tab:results:artifacts}, and are described in \cref{theory:eds:artifacts}.
...
% DONE FW: model background as a spline, and use the mu_rho as a input to refine where the absorption edge could be.
...
% C contamination
The carbon contamination gave raise to a C K$\alpha$ peak, present in all spectra.
...
% DONE FW: investigave the occurance and possible use of M-lines for Z<57. Could at least be useful for TEM, where the absrption in the specimen is lower.
...
% requirements for test specimen
A test specimen for such a routine should be a bulk specimen, as the goal is to assess a SEM setup.
...
% DONE FW: non-flat corrections, which migt be very useful, but probably requires lots of work. First step is getting open-source bulk corrections working properly for flat surfaces, which should be documented well so that further work is easier to do.
...
% DONE FW: additional calibration routine when low energy peaks are of interest.
...
For quantitative analysis, it is best to have a high PT, as the energy resolution is better, and the amount of artifacts is lower.
...
% DONE FW: record voltage series with constant count rate, or with constant current but with PT4. To achieve better quality spectra and further investigate the low voltage spectra.
...
% DONE FW: run DH limit, but only give warning at higher deviations, as giving all parameters always will just result in information overflow. A more sophisticated and complete notebook.
...
The Fiori P/B ratio is a number often given by EDS manufacturers, at least for TEM EDS, where the metric is an indication of how much signal the detector can get in a peak of interest compared to the background.
...
As explained in \cref{theory:xray_formation:characteristic} and stated in Goldstein \cite[Ch. 4.4]{goldstein_scanning_2018}, an element %is relatively transparent to its own characteristic X-rays,
...
% DONE FW: continue to investigate the usefulness of the Fiori ratio in SEM EDS, eg on different acu params to see if it can be used to optimize acuq params.
% DONE FW: Fiori ratio as a measure of peak overlap with varying energy resolution (if the number of counts in the overlapping peaks are the same, relative to the bg, the Fiori number should be the same. Thus, there should be no issue, but if the ratio change, the peaks are not separated).
...
A followup of the DTSA-II software in the newer programming language Julia is available and called "NeXL Spectrum"\footnote{Available at \url{https://pages.nist.gov/NeXLSpectrum.jl/} and \url{https://github.com/usnistgov/NeXLSpectrum.jl}}, but its documentation is still very limited.
...
% AZtec as the "truth"
The results from the AZtec software are quite close to the expected reference values, but should not be considered as the "truth".
There are variations in the results, which is expected in the order of around $2$ at\% in EDS analysis \cite{goldstein_scanning_2018,williams_carter_tem_2009}, but trusting black box software can give a false sense of security.
...
% TON: I check reference and some extra details are give, AZtec, XPP and all data points in SI. On safe side, delete the last sentence. Could be that Nature has rules on how report EDX and in minerology there are often critical questions on EDX. In other fields quant results from commercial might be used without proof.
...
The GaSb spectrum at $5$ kV is quantified far off from the expected reference values, both by AZtec and by the intensity ratio.
...
% DROPPET FW: "This show that quantification routines needs improvement and tailoring for low beam energies."
...
% GaAs
Different $i_b$ is not affecting the correction needs much, but the $E_0$ does.
...
% DONE discuss: the need to increase the weight of Sb. The absorption should be stronger of Ga La, so this is not the reason. Probably a Z effect. Also overvoltage. Maybe omega.
...
Quantification of SEM EDS data with the thin film assumption is done to illustrate the risk of using wrong assumptions, and to test if the Cliff-Lorimer method in HyperSpy with absorption corrections can be used for quantification of SEM EDS data.
In general, the TEM EDS routines gave poor quantitative results for the SEM EDS data of a bulk specimen.
...
As the table shows, the electrons accelerated by $30$ kV can penetrate around $4000$ nm while still have enough energy to excite the relevant X-rays.
...
% DONE FW: "an extensive analysis with multiple different compositions.", not just 50:50.


% GaSb $5$ kV, AZtec chose M line.
One strange difference between the SEM EDS and TEM EDS setting in AZtec is the default choice of Sb line for GaSb at $5$ kV.
...
The EDS detector is equipped with an ultrathin polymer window \cite{xmaxn_datasheet}, which should have a low absorption of X-rays, but the matrix absorption is still high at low energies.
...
It would be possible to manually measure the Sb M$\zeta$ intensity with integration windows, but this was not done as the quality of this $5$ kV spectrum was poor anyway.


% better k-factors?
A major impact on the TEM EDS quantification results is the k-factors used, which is calculated theoretically by AZtec.
The k-factors change with specimen, line, and beam energy, and the exact equation can be found in Williams and Carter \cite{williams_carter_tem_2009}.
If the k-factors could be refined, the TEM quantification results would probably improve.
% How much this improvement would be is just speculations, but t
The results might then be more comparable to the SEM EDS results.
...
Achieving better k-factors can be done by measuring standard specimens on the same microscope as commonly known \cite{goldstein_scanning_2018,goodhew_2001,williams_carter_tem_2009}, although this process can be demanding to do consistently for a broad range of phases and compositions \cite{williams_carter_tem_2009}.
% but then the SEM EDS k-ratio method could be used instead.
...
Secondly, the results presented in \cref{results:quantification_cliff_lorimer} show that applying absorption corrections can improve the quantification results to some degree.
In general, using TEM EDS routines on bulk data is unreliable.
...
As pointed out earlier in this discussion, there are multiple indications showing that the GaAs specimen needs absorption corrections, while the GaSb specimen have other issues, such as strong Z effects.
...
The absorption corrections done with the ZAF model show that dividing the maximum generation depth by $4$ works better than by $2$ or $3$.
...
This is an indication that the quantification based on GaSb spectra have more important corrections than absorption.
...
The most important input parameter is the mass absorption coefficient, $\mu_\rho$.
...
Another reason for the deviation is the implementation of the ZAF correction, which may have bugs in the code.
To trace errors in the code, the code should be tested on a range of unknown standards with different elements and beam energies, as this will reveal how the model behaves on a broader range of data.
...
The implementation is done to the author's best knowledge, but the code should be tested more systematically and be reviewed by other people to ensure that it is bug free.
% , but there is no guarantee that the code is bug free.
Additionally, every model is a simplification of reality, and the models used in this work are no exception.
The assumptions and simplifications made in the ZAF correction model in this work does seem to be on track for GaAs, but still having plenty of room for further improvements.
...
% DONE conc: the assumtion that the two effects of Z cancel each other out is apparently not valid for GaSb.
...
Code bugs are easier to reveal and trace when parts of the code can be tested separately.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/discussion/PAP_phi_of_rhoz.pdf}
    \caption{
...
% This means that $ F = R / (S \cdot Q(U)) $, i.e. that the area of $\phi (\rho z)$ is inversely proportional to the ionization cross-section.
% This conclusion is logical, when viewing the ionization cross-section as the probability for an ionization, and 
...
These plots show that the XPP Python implementation is not perfect yet, but should be close enough to be useful.
...
Two type of bulk corrections from the XPP model were implemented and tested; (i) only corrected for the calculated generation of X-rays, and (ii) corrected for the ratio between the emitted and generated signal, i.e. the absorption factor.
...
The correction with $f(\chi)$ is supposed to correct for both atomic number Z and absorption A, as the calculated absorption factor is dependent on both $F$ and $F(\chi)$, where $F(\chi)$ is the $\phi(\rho z)$ curve with absorption correction in each layer.
$F$ and $F(\chi)$ are illustrated in \cref{fig:discussion:pap:F_and_Fchi}.
The two corrections were tested because of the implications of the previously discussed results, where GaAs need absorption corrections, while GaSb also need corrections for different Z.
...
This is an implication that the Z effects alone are not that strong in GaAs, but the combination of Z and A corrections gave the best results.
% GaSb
...
These numbers are in the same range as the numbers provided by HyperSpy, but there are noticeable deviations.
...
Improvements to the values could be found through simulations, and should be confirmed with empirical measurements.
...
However, the C and O contamination are only on the surface, and their effect is limited.
...
If this is a source of error, it can explain why the AZtec software is getting better results, because Oxford Instruments have gradually refined their input parameters and equations through testing.
...
Even though Oxford Instruments refer to the XPP model in the original PAP paper, the exact equations used in their commercial software is not published.
...
Other correction models, such as the PROZA96 model \cite{bastin_proza96_1998}, should also be tested in future work.
The PROZA96 model can be implemented with the code developed in this work as a starting point, because there are overlapping parts between the models.
...
Other models which are not based on relative intensities, like the PAP and PROZA96 models, could also be tested in parallel in future works.
% DONE discuss: the F and f(chi) is calculated for relative intensities. To calculate absolute number of X-rays generated, the probe current must be know. That was not possible in this project. Calculations of absoule generated X-rays are done in e.g. the Zeta method for thin films.
...
As discussed, the model coded in this project can be improved by refinement of the input parameters, by testing on different specimen, applying restrictions for which corrections to use in which cases, and by refining the code.
...
% DONE FW: refine the XPP model. It works in AZtec pretty well, so it should work, but might need to compensate differently with different compositions.

% DONE FW: do the XPP calculations analytically, might improve the results. Should be possible with standard modern computers.

% DONE FW: test other correction models, like PROZA96. The code here can be a start, as F is reused from the PAP model. Also test absolute models, where the probe current is measured with a Faraday cup.
