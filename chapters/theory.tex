...
% TODO: discuss: can we really neglect F?
...
In this thesis, the background is modelled as a polynomial function of the energy and fitted to the data with a least squares method \cite{hyperspy_1.7.1}.
...
\brynjar{TODO: Insert SE and BSE images of C-film on Cu-grid with Ga. Topology and Z-contrast.}
...
\begin{figure}[pht]
    ...
    \caption{
        ...
        \brynjar{TODO: Consider an alternative source.}
    }
...
To produce a satisfactory amount of X-rays, it is not enough to just have the overvoltage slightly above 1, as the \textbf{ionization cross-section}, $Q$, plays a critical role.
...
% TODO: mention advices U in discussion
...
The \textbf{count rate} is the number of counts per second (cps), and is affected by multiple factors like the beam energy (probability of X-ray creation), beam current (proportional to creations), the solid angle (detector geometry) and the processing capabilities of the electronics.
...
% TODO discuss: refer back to advices DT
...
Carbon contamination can be revealed by a low voltage BSE image, where the carbon will appear as a darker square on the surface.
...
% TODO discuss: the low coincidence peak rates might be due to operation on the low end of the cps that the detector can handle. This was done as the PT was set high. In general not a big issue in the data aqcuired. Or the sofware. Read the ASTM thing below
...
% TODO discuss: mention the ASTM (9.2.4) advice below, i.e. look for escape and sum from the strongest line. If not present here, they are unlikely to be present in other lines.
% TODO discuss: Si Ka is expected to be present and low, since an SDD is used. This is observed.
% TODO discuss: higher energy lines (like Cu Ka), does not have escape peaks in SDD (ASTM 8.3.2)
...
% TODO discuss: software removing escape peaks.
...
\brynjar{TODO: Rethink order. Ton: "For me the logic order would be, Overview, Scale and offset, energy resolution, deviations positions, Fiori, DH as this order you will do it, importance and reading plot left to right."}
...
% TODO suggested standard specimen:
...
If either the metedata is wrong or the specimen is charging, a quantitative analysis will yield poor results \cite{iso_quantification_22309}.
...
%TODO discuss: yes, the advice of higher throughput for a bit lower e-res is nice and confirmed.
...
% TODO discuss: the Fiori P/B increase with E_0 in SEM too, as stated in W&C.
...
% TODO discuss: mu_rho real is also affected by other elements present, e.g. C and O. Thus mislabeling is even a feilkilde here, and the mu_rho used can be off because of this.
...
% TODO theory: illustrate the TOA and (rho z)?
...
% TODO discuss: both multiplying with Q and ignoring Q gave unstable and unreasonalbe results.
