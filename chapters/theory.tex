...
As seen in \cref{eq:theory:xray_formation:characteristic:step2:energy}, the critical ionization energy E$_C$ is slightly higher than the energy of the characteristic X-ray ($h\nu_{\textnormal{X-ray}}$).
%energy of the characteristic X-ray is somewhat lower than the critical ionization energy, E$_C$.
...
The probability of absorption is defined through the \textbf{mass absorption coefficient}, $\mu_\rho$\footnote{The mass absorption coefficient is defined as $\mu/\rho$ \cite{goldstein_scanning_2018}, but is noted as $\mu_\rho$ in this work because of \cref{eq:theory:quantitative:absorption}}, which is a function of energy.
The abrupt increase of $\mu_\rho$ slightly above an characteristic X-ray line is called an \textbf{absorption edge} \cite{goldstein_scanning_2018}.
...
% This absorption effect is usually visualized with a plot of the \textbf{mass absorption coefficient} ($\mu_\rho$), as a function of energy.
...
\cref{fig:mass_absorption_coefficients} shows the mass absorption coefficients for GaAs in panel (a) and GaSb in panel (b), with characteristic X-ray energies marked with vertical lines.
...
\begin{figure}[phtb]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/mass_absorption_coefficients.pdf}
    \caption{
...
If the absorbed X-ray have an energy higher than the E$_C$ + 5 keV of the other lines, the fluorescence effect is negligible \cite[p. 306]{goldstein_scanning_2018}.
...
\begin{figure}[pht]
    ...
    \caption{
        ...
        The spectrum data was aquired in the project report work \cite{project_report}.
    }
...
A higher overvoltage will produce more X-rays, but also decrease the spatial resolution of the analysis as the interaction volume is increased with E$_0$. % ton: fine section
...
The methods selected for the matrix corrections are the absorption correction through the ZAF-method and the PAP algorithm, where the PAP algorithm is more physically correct \cite{pap_1991,bastin_proza96_1998}.
...
Both R and S is implemented in the PAP model \cite{pap_1991}.
...
Where $(ZAF)_i$ is the ZAF-correction for element $i$ and $k_i$ is the k-ratio for element $i$.
...
This calculation is part of the PAP method described below, thus a simpler application of the absorption correction is used for the ZAF correction in this work.
...
\brynjar{To discussion: maybe this approximation is severely wrong. Monte Carlo simulations show a skew towards z=0, and the numbers from HS is big. However, testing different numbers for r did not give any other more satisfactory results.}
...
If a simpler model proves to be sufficient, it will allow users to understand the correction faster and possibly altering it to the specific needs,  while also being easier to implement in HyperSpy.
...
In the original publication, a simplified model called XPP is also presented\footnote{"X" in XPP is probably short for e.g. express, but it is not stated in the paper.}.
...
As it is, allegedly, the XPP model which is used in AZtec \cite{} \brynjar{cite this correctly}\footnote{\url{https://www.oxinst.com/blogs/how-to-overcome-challenges-in-publishing-eds-results}}, it is the XPP and PAP method which is tested in this work.
Other methods, such as the PROZA96 model, could be tested in future works, using the code developed in this work as a starting point, because there are overlapping parts between the models\footnote{One similarity is the calculation of the area of the $\phi(\rho z)$ curve. 
The PROZA96 model refer to the PAP model paper for calculation of this area \cite{bastin_proza96_1998}.}
...
% is not tested in this work, but could be investigated in future works.
...
The $\phi(\rho z)$ curve is the relative emitted intensity as a function of mass depth, where mass depth is the product of density ($\rho$) and path length ($z$).
...
The following is a consice description of the PAP model, detailed enough to implement the model in a Jupyter Notebook. For further details, see the original paper \cite{pap_1991}.
...
The number of primary ionizations of line $l$ generated in atoms A by the incident electron is given by:

% n_a = C_a N^0/A Q_l^A(E_0) F
\begin{equation}
    \label{eq:theory:quantitative:pap:general_principle:n_a}
    n_A = C_A \frac{N^0}{A} Q_l^A(E_0) F
\end{equation}

Where $n_A$ is the number of generated primary ionizations from atoms of element A, $C_A$ is the concentration of element A in mass concentration\footnote{The author have had some frustrating moments because of mixing of wt\% and at\% while working with quantitative EDS.}, $N^0$ is Avogadro's number, $A$ is the atomic weight of element A\footnote{The author of this work assumes $A$ is the atomic weight, but it is not stated explicit in the PAP-paper.}, $Q_l^A(E_0)$ is the ionization cross section of element A at the energy of the incident electron, $E_0$, and $F$ is the integral of $\phi(\rho z)$.
...
The calculations done in the PAP model is divided into two parts: (a) calculate the area F under the $\phi(\rho z)$ curve, and (b) calculate the generated intensity based on a parabolic representation of $\phi(\rho z)$.
...
In equation (2) it is stated that $n_A = C_A (N^0/A) Q_l^A(E_0) F$, and in equation (3) it is stated that $n_A = C_A (N^0/A) (R/S)$.
This implies that $(R/S) = Q_l^A(E_0) F$.
However, equation (13) states that $F = (R/S) \cdot Q_l^A(E_0)$, which is a contradiction to the previous equations.
...
\paragraph*{\emph{The deceleration factor $1/S$}} is a measure of the electrons energy loss as a function of energy in the specimen. $1/S$ is an integral over energy from the incident energy ($E_0$) to the energy of the characteristic X-ray ($E_C$), of the product of the ionization cross section ($Q_l^A(E)$) and the energy loss function ($dE/d\rho s$):

% 1/S = int_E_C^E_0 Q_l^A(E)(dE/d rho s) dE
\begin{equation}
    \label{eq:theory:quantitative:pap:calculation_of_F:S}
    1/S = \int_{E_0}^{E_C} Q_l^A(E) \frac{dE}{d\rho s} dE
\end{equation}
...
\begin{equation}
    ...
    Q_l^A(U) \propto \frac{\ln(U)}{U^m} E_C^2
\end{equation}
...
A plot of the ionization cross section as a function of overvoltage using \cref{eq:theory:quantitative:pap:calculation_of_F:Q_l} is shown in \cref{fig:PAP:ionization_cross_section}.
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        The maximum $Q$ is annotated, which is at the same overvoltage, but different energies due to different E$_C$.
    }
...
\cref{fig:PAP:energy_loss_electrons_dE_drhos} show the average energy loss of the electrons in GaAs and GaSb, according to the PAP model.
...
% \begin{equation}
%     \label{eq:theory:quantitative:pap:calculation_of_F:}
...
\subsubsection{PAP/XPP (b) Generated intensity through parabolic representation of $\phi(\rho z)$}
...
The section covers the necessary calculations to find the generated intensity, which is described as the "Simplified model: XPP".
...
The emergent intensity of a specimen is given by:

% I_A propto C_A Q_l^A(E_0) F(chi)
\begin{equation}
    \label{eq:theory:quantitative:pap:emergent_intensity}
    I_A \propto C_A \cdot Q_l^A(E_0) \cdot F(\chi)
\end{equation}
    
Where $C_A$ and $Q_l^A(E_0)$ is described earlier, and $F(\chi)$ is the parabolic representation of $\phi(\rho z)$, with $ \chi = \mu_\rho \cdot \csc(TOA)$\footnote{$\chi$ is not explicitly defined in the PAP paper, but is taken from Love and Scott \cite[Eq. (1)]{love_scott_1990}, which is a central reference in the PAP paper.}. $F(\chi)$ is given by:
