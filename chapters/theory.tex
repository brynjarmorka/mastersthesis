...
\cref{fig:characteristic_xray_formation} shows in a chematic way the formation of characteristic X-rays with the atom as in the Bohr model, where ionization and relaxation is divided into two steps.
The figure show three possible radiative relaxations, which produce three characteristic X-rays with different energies.
...
The binding energy of the core orbital is E$_C$, \textbf{the critical ionization energy}.
...
The energy required to ionize an electron, E$_C$, can be referred to as the \textbf{absorption edge}, because the absorption an atom does increase abruptly at the energy of the absorption edge.
...
The factors mentioned above are discussed further in the section on quantitative analysis in EDS \cref{theory:quantitative_analysis}.
Calculating the intensity of characteristic X-ray lines from bulk specimen is not trivial, but in Goldstein there is an experimental based equation for this: \cite[Eq. 4.8]{goldstein_scanning_2018}:
...
When a thin specimen (<30 nm) is analyzed \cite{watanabe_williams_zeta_2006}, the absorption effect can be ignored, the so-called thin film approximation used in Cliff-Lorimer quantification, see \cref{theory:quantitative_analysis}.
...
For secondary fluorescence to happen, a characteristic X-ray must be absrobed and it must have higher energy than the critical ionization energy of other lines in the material.
If the absorbed X-ray have a energy higher than the E$_C$ + 5 keV of the other lines, the fluorescence effect is negligible \cite[p. 306]{goldstein_scanning_2018}.
The fluorescence effect is also dependent on the path length of the X-rays in the specimen.
Compensation for the fluorescence effect is discussed in \brynjar{Cref ZAF / PRZ}.
...
% intro
The detectected spectrum contains more than peaks from characteristic X-ray lines as discussed so far.
...
% the energy, 1/E
The energy lost by an electron in a single scattering event can be any value from almost zero to the incident electron energy, E$_0$.
...
\begin{figure}[pht]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/background_absorptionEdge_Si.pdf}
    \caption{
        ...
        The data is from \cite{project_report}.
    }
...
\cref{fig:eds_theoretical2realSpectrum} shows a schematic of the steps from a theoretical spectrum to a observed spectrum of GaAs.
...
Panel (c) shows a observed spectrum, where the unevenness of the recorded countings is visible.
...
\begin{figure}[hbtp]
    ...
    \caption{
        From theory to a observed spectrum.
        (a) a theoretical spectrum with only the characteristic lines, (b) lines broadened to peaks on a background, and (c) a observed spectrum.
...
This section provides a brief overview of how the electron beam is created and controlled in the electron microscope, as well as information on electron-matter interaction, imaging detection and image contrastin SEM.
...
There are many similarities between the above described SEM setup and TEM in scanning mode.
In this work, the focus is on SEM EDS, which on multiple occasions is compared to TEM EDS.
Thus, this section will briefly introduce the TEM setup based on Williams and Carter \cite{williams_carter_tem_2009}, with a focus on the relevant differences to SEM EDS.
...
Unlike SEM, imaging detectors in TEM EDS are placed under the specimen both due to the signal in TEM EDS being weaker due to the small interaction volume and limited space around the TEM specimen to place detectors.
The placement of the detector affect the take-off angle, which is discussed in \cref{theory:eds:hardware}.
...
\brynjar{TODO: KOMMENTARER FRA TON ER RETTET FREM TIL HER}
...
One of the drawbacks of the SDDs compared to the Si(Li) detectors, is the detector efficiency ($\epsilon$), illustrated in \cref{fig:detector_efficiency}.
...
\begin{figure}[pht]
    ...
    \caption{
        Illustration of the detector efficiency, $\epsilon$ of a Si(Li) and an SDD, as a function of the X-ray energy.
...
$\Omega$ is the active area (A) of the detector divided by the square of r, the distance from the sample to the detector.
...
Changing the tilt of the sample changes the TOA, and can increase intensity of the signal.
...
The natural line widths of the X-ray lines treated in this work is around 10 eV, but the peak widths in EDS spectra are around ten times larger.
The shape of the natural lines are Lorentzians, but the electronics broadens the peaks to Gaussians.
This effect is due to the electronics, which handles X-ray pulses corresponding to energies between 0.1 and 40 keV.
...
The acquisition parameters can also affect the energy resolution, mainly due to the process time, discussed more in \cref{theory:eds:user_controlled_parameters}.
...
When the overvoltage for a specimen is discussed, it is refering to the overvoltage for the line of interest with highest critical ionization energy.
...
As seen in the figure, the probability of ionization increase sharply until U is around 4-5, and then it decreases.
...
A higher overvoltage will produce more X-rays, but also decrease the spatial resolution of the analysis as the interaction volume is increased with E$_0$.
...
% beam current
The \textbf{beam current} (I$_0$) is the current of the electron beam, and is measured in nA or pA.
A higher current will produce more X-rays.
Multiple papers and textbooks like Goldstein advise the measurement of the current hitting the specimen, but this has not been possible in the SEMs used in this work.
...
In Goldstein, it is stated that the Si(Li) detectors handles coincidence events better than SDDs, and that 30\% dead time was "close to optimal for all vendors and most samples" for Si(Li) detectors \cite[p. 466]{goldstein_scanning_2018}.
...
Thus, awareness of the possible artifacts is important for a thurough analysis of the spectrum.
...
\brynjar{TODO: Skrive om fluorescence ett eller annet sted.}
...
\ton{Do you want an illustration here of a, b, c, d?}
...
This illustrates one of the challenges with digital solutions implemented by the vendors, as the software is a black box which can enhance the spectrum, but also produce bad results. %can produce bad results.
...
The aim of this work is to improve EDS bulk quantification, and one way to do that is to improve the performance of the EDS setup.
...
Routines for a performance check seems to vary significantly between different laboratories, and the industry has not yet agreed on a standard set of metrics, aside from the energy resolution, measured by the FWHM the Mn K$\alpha$ line.
...
Literature on performance parameters for SEM EDS setups are less common, but some papers are available and the software DTSA-II \cite{software_dtsaii} includes a quality control program, described in the textbook by Goldstein \cite{goldstein_scanning_2018}.
Goldstein also include details on a range of parameters and possible SEM EDS issues.
...
\cref{fig:theory:eds_performance:overview:peak_deviations} shows the deviations in peak positions, which is a affected by the energy scale and offset, and which part of the spectrum is used for calibration.
\cref{fig:theory:eds_performance:overview:strays} shows the effect of stray radiation, which is can be quantified by calculating peak ratios.
...
\begin{figure}[htp]
    ...
    \caption{
        ...
        The charging sampe (red) has a lower effective beam energy than expected (10 keV).
...
The sample should be homogenous, and have a known composition, where the elements present have lines at both low and high energies.
...
The peaks in the spectrum should not overlap too much, as this will make the fitting of the peaks less accurate.
...
When vendors, like Ted Pella who provides the NiO test sample for TEM, are used, the sample is not available immediately, and the delivery time can be long.
...
The effective beam energy is the incident beam energy, E$_0$, that excites X-rays in the specimen.
...
Another explanation of a large deviation is that the voltage indicated by the SEM is wrong.
In both cases, a quantitative analysis will yield poor results \cite{iso_emsa_22029}.
Thus, the Duane-Hunt limit test the acceleration voltage and the conductivity of the specimen.
...
Thompson show that with the dead time kept constant, a higher input count rate gives worse resolution, with the example of 121 eV resolution with <5000 cps, and 140-150 eV resolution with >100,000 cps \cite{keith_energy_res_2013}.
...
The third is to use any peak in the spectrum, and calculate what the energy resolution would be if there was a Mn K$\alpha$ peak in the spectrum.
...
This approach is based on a study in 1995 where five TEM laboratories were given a standard NiO test specimen with a series of EDS test measurements to be done, where one of the tests were focused on the energy resolution.
...
This is, to the knowledge of the author, not commonly done by EDS detector vendors.
\ton{OK to use the previous sentence?}
...
Thus, the user should adjust the trade-off according to the specimen used.
...
If there is a noticable deviation, the user should probably contact the EDS detector vendor for support.
...
The original definition of the Fiori P/B is from a publication by Fiori, Swyt, and Ellis in 1982 \cite{fiori_peak_background_1982} in \emph{Microbeam Analysis 1982}, which is available at the NTNU library, where the definition is:
...
All these arguments were made for TEM, but they are equally true for SEM, and thus the metric is relevant for SEM.
...
This should probably be true for SEM EDS setups too, and are explored in this work.
% TODO: results and discussion on this
TEM EDS signals can have lower relative background intensity compared to peak intensity than SEM EDS signals \ton{Do I need citation here?}.
...
A good reference numbers have not been found for this metric for SEM EDS.
...
The usefulness of using both metrics is explored furhter in this work.
...
\ton{The stuff below is unfinished and more in the style of notes/ideas.}
...
\label{theory:quantitative_analysis}



\brynjar{TODO: Fjerne det om phi-rho-z, fordi jeg bruker PROZA96 (som er en videreføring av phi-rho-z, eller?)}


% Limitations done in this work
This work aims to improve the analysis for SEM EDS data, and this section will focus on the quantitative analysis.
As a master thesis, the scope of this work is limited, and the main focus on the quantitative analysis is to implement matrix corrections in a Jupyter notebook which could be developed further into for example an addition to the HyperSpy package.
As stated in the ASTM 1508 standard, there are multiple different programs available for matrix corrections with some differences, but the accuracy of the EDS analysis is probably not limited by the accuracy of the matrix correction \cite{astm_e1508_eds_quantification}.
The methods selected for the matrix corrections are the absorption correction through the ZAF-method and the PAP algorithm, where the PROZA96 algorithm is more physically correct \cite{bastin_proza96_1998}.
The atomic number correction (Z) in ZAF is excluded, as the correction have two effects with tend to go in opposite directions and cancel each other out \cite[p. 300]{goldstein_scanning_2018}.
These two effects are the backscattering factor (R) and the stopping power (S).
Both R and S is implemented in the PROZA96 algorithm through the PAP model \cite{pap_1991}.
...
This section will start with an introduction to the principle of quantitative EDS analysis, before the different corrections are presented.



\subsection{Principle}
\label{theory:quantitative_analysis:principle}
...
\begin{equation}
    \label{eq:theory:quantitative_analysis:castaing_generated}
...
However, the generated intensity is not the same as the measured intensity, thus Castaing introduced a correction factor applied to the measured intensity ratio:

% C_unknown / C_standard = k * I_unknown / I_standard

\begin{equation}
    \label{eq:theory:quantitative_analysis:castaing_measured}
    \frac{C_\textnormal{unknown}}{C_\textnormal{standard}} = K \frac{I_\textnormal{unknown}}{I_\textnormal{standard}}
\end{equation}

Where $I$ is the measured intensity and $K$ is a correction factor that depends on the atomic number (Z), the absorption (A), and the fluorescence (F) in the sample \cite{goldstein_scanning_2018,williams_carter_tem_2009}.
These corrections for bulk specimen is covered below.
The implementation of satisfactory correction factors is a complicated task, as the X-rays are generated in an interaction volume which depends on the specimen composition and the electron probe conditions.
Castaing and Deschamps established the theory experimentally in 1955, but the method was restricted by the correction calculation as an iterative process. %  \cite{castaing_1955},
Later Philbert derived an analytical expression for iterative routines \cite{philbert_1963}.
Modern matrix corrections are typically divided into two groups: the ZAF-corrections and the $\phi(\rho z)$-corrections.
% ZAF-corrections treat Z, A, and F independently, and is covered in \cref{theory:quantitative_analysis:zaf}.
% The $\phi(\rho z)$-corrections treat the Z and A together in a more complex calculation, and is covered in \cref{theory:quantitative_analysis:phi_rho_z}.


% The Cliff-Lorimer method, for thin samples
One way to avoid the iterative process is to use thin samples, where absorption and fluorescence can be neglected, which is typically the case for TEM specimen.
Cliff and Lorimer showed in 1975 that the intensity from a specimen with two elements can give the composition ratios, without the use of standards.
The Cliff-Lorimer method have a correction factor which is dependent on the instrument settings and the atomic number effects in the specimen.
This correction factor in the Cliff-Lorimer method is named the "k-factor".
...
The k-factors are a correction factor used in the Cliff-Lorimer quantification method for thin samples.
...
\begin{equation}
    \label{eq:theory:quantitative_analysis:cliff_lorimer}
    \frac{C_A}{C_B} = k_{AB} \frac{I_A}{I_B}
\end{equation}

Where $k_{AB}$ is the k-factor for the two elements $A$ and $B$.
The k-factor is most accuratly determined experimental, but can be calculated theoretically.


The k-ratio method is a quantification method for bulk samples, where the k-ratio is the ratio between the measured intensity of the unknown and the standard.
The k-ratio method includes the ZAF-correction.
Element $i$ is quantified through the equation:

\begin{equation}
    \label{eq:theory:quantitative_analysis:k_ratio}
    \frac{C_\textnormal{i, unknown}}{C_\textnormal{i, standard}} = (ZAF)_i \frac{I_\textnormal{i, unknown}}{I_\textnormal{i, standard}} = (ZAF)_i \cdot k_i
\end{equation}

Wher $(ZAF)_i$ is the ZAF-correction for element $i$ and $k_i$ is the k-ratio for element $i$.
As with the k-factor, the k-ratio is most accuratly determined experimentally, but can be calculated theoretically.
The AZtec software from Oxford Instruments can both provide the k-factor and the k-ratio for the elements in an analyzed sample, which are theoretically calculated.
...
\begin{equation}
    \label{eq:theory:quantitative_analysis:zaf_a_b_relation}
    ZAF_A = \frac{1}{ZAF_B}
\end{equation}

In this work, $ZAF_B$ is used.
...
\label{theory:quantitative_analysis:zaf:absorption_correction}
...
The absorption reduce the measured intensity of X-rays from a certain point by:

% I = I_0 * exp(-mu * rho * t)
\begin{equation}
    \label{eq:theory:quantitative_analysis:absorption}
    I = I_0 \cdot e^{-\mu_{\rho} \cdot \rho \cdot t}
\end{equation}

Where $I$ is the measured intensity, $I_0$ is the generated intensity, $\mu_{\rho}$ is the mass absorption coefficient\footnote{The symbol for mass absorption coefficient is $(\mu/\rho$) in Goldstein, but is written as ($\mu_{\rho}$) in this work, because writing $(\mu/\rho)\cdot \rho$ not ideal.}, $\rho$ is the density of the specimen, and $t$ is the path length of the X-ray in the specimen.
...
% t = z *csc(TOA)
\begin{equation}
    \label{eq:theory:quantitative_analysis:path_length}
...
This calculation is part of the PROZA96 method described below, thus a simpler application of the absorption correction is used for the ZAF correction in this work.
Instead of applying the absorption correction for each point in the interaction volume, the absorption is calculated for the average point in the interaction volume.
The average point selected in this work is the maximum path length of an incident electron, divided by two, where the electron range is calculated by HyperSpy which have implemented the Kanaya-Okayama parameterization \cite{kanaya1972,goldstein_scanning_2018}.
This should be an approximation of the center of the interaction volume.
\brynjar{To discussion: maybe this approximation is severely wrong. Monte Carlo suimulations show a skew towards z=0, and the numbers from HS is big. However, testing different numbers for r did not give any other more satisfactory results.}
This simplification is partly justified by the fact that the avaliable measured intensity $I$ is the sum of the intensity from all points in the interaction volume.
...
If a simpler model proves to be sufficient, it will allow users to understand the correction faster and possibly altering it to the spesific needs,  while also being easier to implement in HyperSpy.



\subsection{The PROZA96 model}
\label{theory:quantitative_analysis:proza96}

The PROZA96 model is a more physically based model for matrix corrections, published by Bastin, Dijkstra, and Heijligers in 1996, as an improvement to the the same authors previous model from 1988 \cite{bastin_proza96_1998,bastin_proza_1988}.
It is an iterative model which calculate the $\phi(\rho z)$ curve as a double Gaussian.
The $\phi(\rho z)$ curve is the relative generated intensity as a function of mass depth, where mass depth is the product of density ($\rho$) and path length ($z$).
...
% \subsection{Alternative quantification}
