...
\brynjar{Quantification, ZAF and PhiRhoZ will be added.}
...
\ton{I am somewhat unsure about the order here. Does it make sense to explain generation of X-rays before the SEM and EDS system?}
...
The energy required to ionize an electron, E$_C$, can be referred to as the absorption edge, because the absorption an atom can do increase abruptly at the energy of the absorption edge.
...
The emitted X-rays, on the other hand, are emitted in all directions and can typically penetrate up to 4000 nm. %, making them the signal in EDS. 
...
The fluorescence yield is heavily dependent on the ionized atom, with in general higher yields for heavier atoms.
...
\begin{figure}[ht]
    ...
    \caption{
        ...
        The figure is adapted from \cite[Fig. 4.3 (d)]{goldstein_scanning_2018}.
    }
...
\begin{figure}[p]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/Sb_L-peaks_30kV_50pA.pdf}
    \caption{
...
The transition $\textnormal{L}_1 \rightarrow \textnormal{K}_1$ has $\Delta l = \Delta j = 0$ and is thus forbidden by the selection rules, see \cref{eq:theory:selectionrules}.
...
Trusting a source blindly can lead to mislabeling in the qualitative analysis, which is discussed in \brynjar{Ref to discussion on this}.
...
% TODO: include something about the escape depth of the X-rays, which increases with energy.
Additionally, the intensity of detected X-rays is not the same as the intensity of the X-rays which are generated.
The probability of absorption increase with density and with the travel distance.
Photons with lower energy are absorbed more easily than photons with higher energy, i.e. the escape depth of the X-rays increase in general with energy, but is also dependent on the absorption edges present.
...
\begin{figure}[pht]
    ...
    \caption{
        ...
        The spectrum shows the overlapping Si K$\alpha$ and K$\beta$ peaks, the Si sum peak and a small O K$\alpha$ peak.
        ...
        \brynjar{NTNU NanoLab stuff \dots}
    }
...
The formation of background radiation is anisotropic, with more signal in the direction that the electrons are travelling.
...
\subsection{From theory to a real spectrum}
\label{theory:eds:fromtheorytoreal}

This last part of the section combines the different parts of the theory to the real spectrum.
\cref{fig:eds_theoretical2realSpectrum} shows a schematic of the steps from a theoretical spectrum to a real spectrum of GaAs.
Panel (a) shows a theoretical spectrum with only the characteristic lines.
Panel (b) shows the lines broadened to Gaussian peaks on a background, where the background is modelled as a polynomial function of the energy.
Notice how the peaks broaden more with higher energy.
Panel (c) shows a real spectrum, where the unevenness of the recorded countings is visible.
The real spectrum is recorded in a SEM with an EDS detector, which is discussed in \cref{theory:sem} and \cref{theory:eds} respectively.
The real spectrum also shows some additional artifacts, like a noise peak and a small Si K$\alpha$ peak at 1.74 keV for the internal fluorescence peak.
...
\begin{figure}[hbtp]
    ...
    \caption{
        From theory to a real spectrum.
        (a) a theoretical spectrum with only the characteristic lines, (b) lines broadened to peaks on a background, and (c) a real spectrum.
...
Both SEMs and TEMs are often equipped with an EDS detector to provide analytical capabilities that complement imaging and diffraction analyzes.
...
A summary of the section is provided in \cref{tab:sem}.
...
\brynjar{Mention Li escape depth in range of tens of nm? Keith Thompson, Is Energy Resolution Still an Important Specification in EDS?}
...
The final SE image is a combination of all the SE classes, but it is the SE signal from the sample which give the topological contrast.
Other examples of signal absorption is the generated X-ray strays due to fluorescence, which is covered in \brynjar{cref to chapter on strays. OBS! Fluorescence is not covered yet}.
...
A SE image is the 2D representation of a set of spectra, where each pixel has one 1D spectrum with the number of SE electrons detected as a function of energy.
...
The next section covers parameters used to quantify the performance of the EDS system. %and the quality of a spectrum.
% based on Goldstein
...
In SDDs the anode size can be kept constant while increasing the area of the active layer, without increased capacitance \cite{notaros_electromagnetics_2010}.
...
\begin{figure}[pht]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/detector_efficiency_illustration.pdf}
    \caption{
...
Having an overvoltage below 1 results in no X-rays generated for the line of interest, which is sees as the line starts at 1.
...
This is reflected in the detectors process time settings, which have to be different for a detector that can reach 10E4 cps compared to a detector that can reach 10E6 cps.
...
Another artifact with give similar issues as coincidence peaks is \textbf{stray radiation, or spurious X-rays}.
...
Strays originating from the (d) the detector results in both the \textbf{internal fluorescence} peak and \textbf{escape peaks}.
...
In Goldstein at page 274, it is shown that an escape peak was present using the 2011 software, but digitally removed in the 2014 software \cite[Fig. 18.7]{goldstein_scanning_2018}.
...
Vendors of EDS detectors like Oxford Instruments provide a guide for how to calibrate the detector, but these guides are limited to calibration of energy resolution and scale \cite{aztec_manual}.
...
Before the test characteristics are described, an overview is given and a summary table is presented in \cref{tab:eds_performance_parameters}.
...
\subsection{Overview of the performance parameters}
...
The performance parameters below can be used to answer these questions.
\cref{tab:eds_performance_parameters} give a brief introduction to each parameter, how it is measured, what values are acceptable, and what it can reveal.
...
Further, detectors with possibilities for high count rates should also specify the energy resolution at a high count rate, e.g. 10E6 cps.
...
Some modern detectors can acquire extremely high counts per second, partly because of the larger sensor sizes, like the newest Oxford Instruments detector Ultim Max EDS with up to 170 mm$^2$, which supposedly can acquire up to 1.5 10E6 counts per second \cite{oxford_ultim_max}.
...
This is also true for change in peak broadening due to different detector settings, like changing increasing the count rate.
...
The problem with subjective choice of integration window, because of the lack of an agreed test standard \cite{williams_standard_definitions_1986}, is exemplified in the EDAX Insight newsletter from September 2018 \cite{edax_insight_2018}, where they have an illustration of the Fiori P/B where the background windows are > 2 keV away from the peak.
...
TEM EDS signals can have lower relative background intensity than SEM EDS signals \ton{Do I need citation here?}.
...
However, it might be possible to use the Fiori P/B metric to optimize the acquisition settings for specific specimen.
...
However, detecting the contamination is done by comparing the peak ratio over time, and if the ratio is falling, it is an indication of carbon contamination.
...
\subsection{Background portion}
\label{theory:eds_performance:bacgrkound_portion}
...
\begin{equation}
    ...
    \frac{\text{Background counts}}{\text{Total counts}} \times 100\%
\end{equation}



% \subsection{Stray radiation measurements - when possible}
% \label{theory:eds_performance:stray}
...
\section{Quantitativt analysis in EDS}
\label{theory:quantitative_analysis}


\brynjar{Finish this section later.}


% the intensity of different elements
As the intensity of the X-rays is proportional to the number of atoms of the given element, the intensities can be compared to find the relative abundance of the different elements.
In the ideal case, the concentration of the different elements are the same as the relative intensity of the X-rays in their corresponding characteristic X-ray lines.

% C_a / C_b = I_a / I_b
\begin{equation}
    \label{eq:theory:xray_formation:simple_intensity_ratio}
    \frac{C_a}{C_b} = \frac{I_a}{I_b}
\end{equation}

However, the intensity is affected by multiple factors mentioned above, so the intensity ratio needs a correction factor.
Multiple of the factors fall out, as they are present in both the numerator and denominator.
The factors that are left are the ones which are dependent on the atomic number.