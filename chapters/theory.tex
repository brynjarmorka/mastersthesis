...
\brynjar{Sentence about quantification, ZAF and $\phi (\rho z)$ will be added.}
...
% \brynjar{TODO? Legge til kommentar om intensity, som er inkludert i Siegbahn med ikke i international. Som gir Siegbahn en praktisk fordel.}
...
\brynjar{TODO: Add average, best, and average of three best, or some more to the results.}
...
Additionally, HyperSpy have implementations of the Zeta method \cite{watanabe_williams_zeta_2006} and the cross-section quantification approach \cite{MacArthur_crosssection_2016}.
...
\brynjar{TODO, from Ton: "Yes, but not here. Introduce ratio from formulas above or from Goldstein and then compare the two as a halfway summary."}
...
\brynjar{From Ton: "could give the formula for the k-factor, Williams and Carter give it for example, and then discus that it has a setup part (detector size, efficiency) and a physics side (cross-section for processes)"}.
...
As it is, allegedly, the XPP model which is used in AZtec \footnote{\url{https://www.oxinst.com/blogs/how-to-overcome-challenges-in-publishing-eds-results}}\brynjar{cite this blogpost correctly}, it is the XPP model which is tested in this work. % TODO
...
The calculations done in the PAP model is divided into two parts: (a) find the \emph{generated intensity} by calculating the area $F$ under the $\phi(\rho z)$ curve, and (b) calculate the \emph{emitted intensity} $F(\chi)$ based on a parabolic representation of $\phi(\rho z)$ with an absorption correction.
...
% \brynjar{TODO: F is R/S / Q}


Now the PAP-paper is at times somewhat confusing, both because the lack of explanation of some symbols, and because of slightly contradicting equations.
...
The author of this work assumes that the equation (13) is incorrect, but both variations for the calculation of $F$ is tested in the Jupyter Notebook.
...
\paragraph*{\emph{The deceleration factor $1/S$}} is a measure of the energy loss of the electrons as a function of energy in the specimen. $1/S$ is an integral over energy from the incident energy ($E_0$) to the energy of the characteristic X-ray ($E_C$), of the product of the ionization cross-section ($Q(U)$) and the energy loss function ($dE/d\rho s$):
...
\paragraph*{\emph{The backscattering loss factor $R$}} is the fraction of the electrons that can generate X-rays, i.e. are not backscattered, calculated from the equation:
...
$\bar{W}$ is the mean reduced energy of the backscattered electron, $\bar{Z}_b$ is the weighted mean atomic number, and $U_0$ is the overvoltage.
...
Where $A$, $B$, $a$ and $b$ are parameters of the parabolic representation of $\phi(\rho z)$, and $\phi(0)$ is the surface ionization. The absorption correction factor can then be calculated by:
...
If the author understands this correctly, the comment is aimed at computational issues in the 90s where values too close to zero would cause numerical issues.
