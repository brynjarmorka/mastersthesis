...
Compensation for the fluorescence effect is discussed in \cref{theory:quantitative_analysis:principle}.
...
\label{theory:quantitative_analysis:pap}

\brynjar{Unfinished. I use PAP or XPP, not PROZA96.}


Pouchou and Pichoir published their PAP model in 1991 for bulk corrections\footnote{"PAP" is short for "Pouchou and Pichoir" \cite{pap_1991}.}.
...
Alternative versions of matrix corrections which build on the PAP model is available, such as the PROZA96 model.
...
It is an iterative model which calculate the $\phi(\rho z)$ curve as a double Gaussian, which is somewhat similar to the PAP model.
As it is, allegedly, the XPP model which is used in AZtec \cite{} \brynjar{cite}\footnote{\url{https://www.oxinst.com/blogs/how-to-overcome-challenges-in-publishing-eds-results}}, the PROZA96 model is not tested in this work, but could be investigated in future works.


\emph{From the paper: "We believe that, rather than treating separately the improvement of corrections for
    ...
    calculation of emergent x rays. For this we have used the usual fundamental physical
    concepts applied in x-ray microanalysis and we apply to them as accurate a description of
    the depth distribution, $\phi$(pz) as possible."}


The PAP model is based on the following steps:

\brynjar{Insert steps in PAP.}
