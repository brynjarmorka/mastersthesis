...
In SDDs the anode size can be kept constant while increasing the area of the active layer, which makes the SDDs scalable in detection size withouth increasing the capacitance \brynjar{ref el.mag.}, which would limit both resolution and throughput \cite[Ch. 16.3.9]{goldstein_scanning_2018}.
...
\begin{figure}[ht]
    ...
    \caption{
        ...
        Remake of \brynjar{find one, eg. https://www.globalsino.com/EM/page4655.html}
...
\brynjar{Ref to where coincidence counts are explained in more detail?}
...
\ton{Also metion that energy resolution is dependent on time live?}
...
This approach is the recommended method provided by Ted Pella Inc., who manufactures standardized NiO test specimens for TEM \verb|\cite{}| \brynjar{citation to Ted Pella sheet? Or Maris reference?}.
...
The second step utilize an equation from Fiori and Newbury from 1978 (\brynjar{original: Fiori, C. E., and Newbury, D. E. (1978). In SEM/1978/I, SEM, Inc., AFM O'Hare, Illinois, p. 401.}), which use one known line to estimate what the FWHM would be at another energy, e.g. Mn K$\alpha$:
...
The method works well when the reference line is a well defined peak. \brynjar{Define "well defined" \dots}
...
Some modern detectors can acquire extremely high counts per second \brynjar{ref?}, partly because of the larger sensor sizes, like the newest Oxford Instruments detector Ultim Max EDS with up to 170 mm$^2$, which supposedly can acquire up to 1.5 million counts per second \brynjar{Fix citation, https://nano.oxinst.com/products/ultim-max}.
...
\brynjar{Look into the 1982 and 1986 papers for more details.}


% TODO: arguments for why it is relevant for SEM
\brynjar{TODO: Paragraph about why it is relevant for SEM.}
...
Even though the definition of the Fiori P/B is simple, in the 80s and 90s both the P and the B had to be estimated with integration windows, as isolating the peak from the background was not trivial.
...
This problem with robustness is exemplified in the EDAX Insight newsletter from September 2018, where they have an illustration of the Fiori P/B where the background windows are > 2 keV away from the peak, which is not a good choice \brynjar{ref both newsletter and Williams, who claims the window should be next to the peak}.
